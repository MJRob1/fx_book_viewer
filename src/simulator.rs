...
pub fn get_configs(configs: &mut Vec<Config>) -> Result<(), AppError> {
    ...
    for i in &parameters {
        ...
        if index > 0 {
            ...
            let liquidity_provider = crate::aggregator::get_str_field(fx_params.next())?;
            let currency_pair = crate::aggregator::get_str_field(fx_params.next())?;
...
pub fn get_local_str_field(field: Option<&str>) -> Result<&str, AppError> {
